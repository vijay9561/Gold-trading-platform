...
$loan=$this->db->query("select *from GOLD_PARCHASE_MASTER where ID='$loan_id' and USERID='$userid'")->row();
$html = '<span>'
        . '<br><span style="font-size:14px;text-align:left;color:gray;">Suvarnasiddhi Gold'
        . '</span>'
        . '</span>';
// output the HTML content
//$pdf->writeHTML($html, true, false, true, false, '');

$html = '<span style="text-align:center;font-size:20px;text-transform:uppercase;">Suvarnasiddhi (INR Deposit) INVOICE'
...
// output the HTML content
$NETAMOUNT=$loan->TOTAL_AMOUNT-$loan->GST_AMOUNT;
$html = '<br><br><br><span style="text-align:center;font-size:18px;font-weight:bold;text-decoration:underline;">Deposit INR Particulars</span><br><br><div style="">'
        . '<br><br>'
        . '<span><table border="1" style="border-collapse: collapse; width:600px; padding:10px; color:#000; text-align:left; font-size:12px;">'
       . '<tr><th>Transaction ID : </th><td>' .$loan->TXTID. '</td></tr>'
        . '<tr><th>Deposit Date : </th><td>' .date('d-m-Y', strtotime($loan->DATES)) . '</td></tr>'
 
        .'<tr><th>Net Deposit amount:&nbsp;</th><td>INR ' .$loan->TOTAL_AMOUNT. '</td></tr>'
        .'<tr><th>Total Deposit amount:&nbsp;</th><td>INR ' .$loan->TOTAL_AMOUNT. '</td></tr>'
        . '</table></span>';
       $html.='</div>';
...
$pdf->Output('Suvarnasiddhi_Invoice-' .$loan->PAYMENT_ID. '.pdf', 'I');
